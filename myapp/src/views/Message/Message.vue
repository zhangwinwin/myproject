  ...
  <div class="message">
    ...
    <div class="msg-box" v-show="tabIndex == 0">
      <ul class="list" v-if="list.has_read_messages">
        <li v-for="(item, $index) in list.has_read_messages" :key="$index">
          <router-link class="user" :to="'/user/' + item.author.loginname">
          <div class="user-heading" :style="{backgroundImage: 'url(' + item.author.avatar_url + ')'}"></div>
          </router-link>
          <div>
    ...
    <div class="msg-box" v-show="tabIndex == 1">
      <ul class="list" v-if="list.hasnot_read_messages">
        <li v-for="(item, $index) in list.hasnot_read_messages" :key="$index">
          <router-link class="user" :to="'/user/' + item.author.loginname">
            <div class="user-heading" :style="{backgroundImage: 'url(' + item.author.avatar_url + ')'}"></div>
          </router-link>
           <div>
